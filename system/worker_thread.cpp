...
void WorkerThread::send_key()
{
    ...
    for (uint64_t i = 0; i < g_node_cnt + g_client_node_cnt + g_repl_cnt * g_node_cnt; i++)
    {
        ...
        cout << "Sending ED25519: " << g_public_key << " to " << to_string(i) << endl;
#endif
        fflush(stdout);
        ...
        cout << "Sending CMAC " << cmacPrivateKeys[i] << " to " << to_string(i) << endl;
        dest.clear();
        fflush(stdout);
#endif
...
void WorkerThread::create_and_send_batchreq(ClientQueryBatch *msg, uint64_t tid)
{
    ...
    else if (g_node_id >= g_node_cnt + g_client_node_cnt) {
